...
public class IdCardDesign {
    public Table whiteBackgroundDesign(Library lib, String path, EmpDetails emp,String validDate){
         ...
         try {
            ...
            // add paragraph
            //company Details;
            String companyName = "cobra industrial security force (i) ltd.\n";
            String companyLocation =" Samvedan, R. K. Singh (Retired IPS) Gujral Path,\n"+
                    "Ravikar Lane, KeshriNagar, Patna - 24\n";
            String companyMob = "Mob.: 9931616742, 9955357967\n";
            String companyReg = "Regd.No.30-09355 : ISO 9001-2015 Certified Company\n";
...
//            Color color_red =  CalRgb.createColorWithColorSpace(new float[]{255f,0f,0f});
            ...
            Text companyNameTxt = new Text(companyName.toUpperCase()).setFont(boldTxt).setFontSize(fontSize).setFontColor(color_deep_blue);
            Text companyLocationTxt = new Text(companyLocation).setFont(boldTxt).setFontSize(fontSize-2).setFontColor(color_deep_blue);
            Text companyMobTxt = new Text(companyMob).setFont(boldTxt).setFontSize(fontSize-2).setFontColor(color_deep_blue);
            Text companyRegTxt = new Text(companyReg).setFont(regRobotoFont).setFontSize(fontSize-2).setFontColor(color_red);
             //sign

            Text empSignTxt =new Text(sign[0]).setFont(boldTxt).setFontSize(fontSize-1).setFontColor(color_deep_blue);
            Text validToTxt =new Text(validDate+"\n"+sign[1]).setFont(boldTxt).setFontSize(fontSize-1).setFontColor(color_red);
            Text authSignTxt =new Text(sign[2]).setFont(boldTxt).setFontSize(fontSize-1).setFontColor(color_deep_blue);
...
//            doc.setMargins(10, 10, 10, 10);
            ...
            companyHeadTable.addCell(new Cell().add(companyDetails).setTextAlignment(TextAlignment.CENTER).setBorder(Border.NO_BORDER));
            ...
            companyFooterTable.addCell(new Cell().add(empSignPara).setPaddingTop(20).setTextAlignment(TextAlignment.CENTER).setBorder(Border.NO_BORDER));
            companyFooterTable.addCell(new Cell().add(validPara).setPaddingTop(5).setTextAlignment(TextAlignment.CENTER).setBorder(Border.NO_BORDER));
            companyFooterTable.addCell(new Cell().add(authSignPara).setPaddingTop(20).setTextAlignment(TextAlignment.CENTER).setBorder(Border.NO_BORDER));
...
//            doc.add(companyFooterTable);
            ...
            main.addCell(new Cell().add(companyBodyTable).setBorderBottom(Border.NO_BORDER).setBorderTop(Border.NO_BORDER));
...
//		}
    ...
    private Table employeeDetailTable(EmpDetails emp,Image gradientImage,PdfFont boldTxt,int fontSize,Color color_deep_blue){
        ...
        empDetailsTable.addCell(new Cell().add(employeeTables.getEmpNameAndAddress(emp,boldTxt,fontSize,color_deep_blue)).setBorder(Border.NO_BORDER));
        empDetailsTable.addCell(new Cell().add(employeeTables.getEmpPost(emp,boldTxt,fontSize,color_deep_blue)).setBorder(Border.NO_BORDER));
        empDetailsTable.addCell(new Cell().add(employeeTables.getDobAndBloodGroup(emp,boldTxt,fontSize,color_deep_blue)).setBorder(Border.NO_BORDER));
